...
public class MailSenderService {
    ...
    private static final String ORGANIZATION_PICTURE = "organizationPicture";
    ...
    /**
     * Send organization invite link.
     *
     * @param user User
     *
     */
    public void sendOrganizationInvite(String email, Organization organization) {
        try {
            log.info(String.format("[EmailService] Sending organization invite e-mail: %s ", email));
            ...
            send(new InternetAddress(senderAddress, appName), new InternetAddress(email, email.split("@")[0]),
                    subject, templateEngine.process("mail/invite-user.html", ctx));

            log.info(String.format("[EmailService] Sent reset password e-mail: %s", email));
        } catch (UnsupportedEncodingException | MessagingException | MailException e) {
            log.error(String.format("[EmailService] Failed to send reset password e-mail: %s", e.getMessage()));
        }
    }
